...
import os, asyncio, logging, random, time, pathlib
...
def _subcategory_names(prefix: str = "", limit: int = 25) -> List[str]:
    with get_session() as db:
        ...
        return [f"{s.category.name} ▸ {s.name}" for s in q.join(Category).order_by(Category.name, Subcategory.name).limit(limit).all()]
...
async def _post_next_card(channel, user_id: int, scope: str, category_id: Optional[int], subcategory_id: Optional[int], points: int, streak_val: int):
    with get_session() as db:
        ...
        if scope == "subcategory" and next_card.subcategory:
    ...
    active_reviews[new_msg.id] = {"user_id": user_id, "scope": scope, "card_id": next_card.id, "category_id": category_id, "subcategory_id": subcategory_id}
    return new_msg, next_card

async def _score_and_advance(channel, old_message, user_id: int, scope: str, category_id: Optional[int], subcategory_id: Optional[int], card_id: int, correct: bool):
    with get_session() as db:
        card = db.query(Card).filter(Card.id == card_id).one_or_none()
        if not card:
            if old_message:
                try: await old_message.edit(content="Review expired.", embed=None, view=None)
        ...
        if score.points >= 100:
            ...
            if reward:
                try:
                    if reward.lower().startswith(("http://", "https://")): await channel.send(reward)
                    else: await channel.send(file=discord.File(reward))
                except Exception: logging.exception("Failed to send reward video")
            scope_label = "All Categories" if scope == "all" else (f"{card.category.name}" if scope=="category" else f"{card.category.name} ▸ {card.subcategory.name}")
        ...
        new_msg, _ = await _post_next_card(channel, user_id, scope, category_id, subcategory_id, score.points, streak.current_streak)
    if old_message:
        try: await old_message.edit(view=None)
        except Exception: pass
...
class ReviewView(discord.ui.View):
    def __init__(self, user_id: int, scope: str, category_id: Optional[int], subcategory_id: Optional[int], card_id: int):
        ...
        self.user_id = user_id; self.scope = scope; self.category_id = category_id; self.subcategory_id = subcategory_id; self.card_id = card_id
    async def _handle(self, interaction: discord.Interaction, correct: bool):
        if interaction.user.id != self.user_id:
            await interaction.response.send_message("This isn’t your review session.", ephemeral=True); return
        try: await interaction.response.defer()
        except discord.InteractionResponded: pass
        await _score_and_advance(interaction.channel, interaction.message, self.user_id, self.scope, self.category_id, self.subcategory_id, self.card_id, correct)
    @discord.ui.button(label="✅ Correct", style=discord.ButtonStyle.success)
    async def btn_correct(self, interaction: discord.Interaction, button: discord.ui.Button): await self._handle(interaction, True)
    @discord.ui.button(label="❌ Incorrect", style=discord.ButtonStyle.danger)
    async def btn_incorrect(self, interaction: discord.Interaction, button: discord.ui.Button): await self._handle(interaction, False)
...
class EditCardModal(discord.ui.Modal, title="Edit Card"):
    def __init__(self, opener_user_id: int, card_id: int, original_message: discord.Message, title: str):
        super().__init__(timeout=300); self.opener_user_id = opener_user_id; self.card_id = card_id; self.original_message = original_message; self.title_text = title
        with get_session() as db:
            c = db.query(Card).filter(Card.id == self.card_id).one_or_none()
            q_val = c.question if c else ""; a_val = c.answer if c else ""
        ...
        self.add_item(self.q); self.add_item(self.a)
    async def on_submit(self, interaction: discord.Interaction):
        if interaction.user.id != self.opener_user_id: await interaction.response.send_message("You didn’t open this card.", ephemeral=True); return
        new_q, new_a = str(self.q.value).strip(), str(self.a.value).strip()
        with get_session() as db:
            c = db.query(Card).filter(Card.id == self.card_id).one_or_none()
            if not c: await interaction.response.send_message("This card no longer exists.", ephemeral=True); return
            c.question, c.answer = new_q, new_a; db.commit(); db.refresh(c)
        try: await self.original_message.edit(embed=_embed_card_display(self.title_text, c), view=CardManageView(self.opener_user_id, self.card_id, self.title_text))
        except Exception: pass
...
class ConfirmDeleteView(discord.ui.View):
    def __init__(self, opener_user_id: int, card_id: int):
        super().__init__(timeout=60); self.opener_user_id = opener_user_id; self.card_id = card_id
    @discord.ui.button(label="Confirm Delete", style=discord.ButtonStyle.danger)
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.opener_user_id: await interaction.response.send_message("You didn’t open this card.", ephemeral=True); return
        with get_session() as db:
            c = db.query(Card).filter(Card.id == self.card_id).one_or_none()
            if not c: await interaction.response.send_message("Already deleted.", ephemeral=True); return
            db.delete(c); db.commit()
        await interaction.message.edit(content="🗑️ Card deleted.", embed=None, view=None)
        await interaction.response.send_message("Deleted.", ephemeral=True)
    @discord.ui.button(label="Cancel", style=discord.ButtonStyle.secondary)
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer(); await interaction.message.edit(view=None)

class CardManageView(discord.ui.View):
    def __init__(self, opener_user_id: int, card_id: int, title: str):
        super().__init__(timeout=900); self.opener_user_id = opener_user_id; self.card_id = card_id; self.title = title
...
def _chunk(lst: List, size: int) -> List[List]: return [lst[i:i+size] for i in range(0, len(lst), size)]

class ListCardsButtonsView(discord.ui.View):
    PAGE_SIZE = 10
    def __init__(self, user_id: int, pairs: List[tuple[int, str]], title: str):
        super().__init__(timeout=900); self.user_id=user_id; self.title=title; self.pages=_chunk(pairs, self.PAGE_SIZE) or [[]]; self.page_index=0; self._rebuild()
    def _rebuild(self):
        for it in list(self.children): self.remove_item(it)
        for cid, q in self.pages[self.page_index]:
            ...
            async def _cb(interaction: discord.Interaction, card_id=cid):
                if interaction.user.id != self.user_id: await interaction.response.send_message("This list belongs to someone else.", ephemeral=True); return
                with get_session() as db:
                    card=db.query(Card).filter(Card.id==card_id).one_or_none()
                    if not card: await interaction.response.send_message("That card was not found.", ephemeral=True); return
                    scope_title=card.category.name if card.category else "Cards"
                    if card.subcategory and card.category: scope_title=f"{card.category.name} ▸ {card.subcategory.name}"
                    embed=_embed_card_display(scope_title, card)
                view=CardManageView(self.user_id, card_id, scope_title); await interaction.channel.send(embed=embed, view=view)
                try: await interaction.response.defer()
                except discord.InteractionResponded: pass
        ...
        if len(self.pages)>1:
            ...
            async def prev_cb(interaction):
                if interaction.user.id != self.user_id: await interaction.response.send_message("This list belongs to someone else.", ephemeral=True); return
                self.page_index=(self.page_index-1)%len(self.pages); self._rebuild(); await interaction.response.edit_message(view=self)
            async def next_cb(interaction):
                if interaction.user.id != self.user_id: await interaction.response.send_message("This list belongs to someone else.", ephemeral=True); return
                self.page_index=(self.page_index+1)%len(self.pages); self._rebuild(); await interaction.response.edit_message(view=self)
            prev_btn.callback=prev_cb; next_btn.callback=next_cb; self.add_item(prev_btn); self.add_item(page_label); self.add_item(next_btn)
...
async def on_ready():
    ...
    try:
        if GUILD_ID_RAW:
            gid=int(str(GUILD_ID_RAW).strip()); await bot.tree.sync(guild=discord.Object(id=gid))
        else: await bot.tree.sync()
    except Exception as e: logging.error(f"Failed to sync: {e}")
...
async def on_raw_reaction_add(payload: discord.RawReactionActionEvent):
    try:
        ...
        state=active_reviews.get(payload.message_id); if not state: return
        if state["user_id"] != payload.user_id: return
        emoji=str(payload.emoji); if emoji not in ("✅","❌"): return
        now=time.time(); key=(payload.message_id,payload.user_id)
        if _last_handle.get(key,0)+0.8>now: return
        _last_handle[key]=now
        channel = bot.get_channel(payload.channel_id) or await bot.fetch_channel(payload.channel_id)
        correct = (emoji=="✅"); user_id=payload.user_id
        scope=state["scope"]; category_id=state["category_id"]; subcategory_id=state["subcategory_id"]; card_id=state["card_id"]
        old_message=None
        try:
            ch=channel
            if isinstance(ch,(discord.TextChannel, discord.Thread)): old_message=await ch.fetch_message(payload.message_id)
        except Exception: pass
        await _score_and_advance(channel, old_message, user_id, scope, category_id, subcategory_id, card_id, correct)
        active_reviews.pop(payload.message_id, None)
    except Exception: logging.exception("raw_reaction handler error")

@bot.tree.command(description="Add or manage categories/subcategories and cards")
...
async def addcategory(interaction: discord.Interaction, name: str):
    with get_session() as db:
        ...
        if existing: await interaction.response.send_message(f"Category **{existing.name}** already exists.", ephemeral=True); return
...
async def editcategory(interaction: discord.Interaction, old_name: str, new_name: Optional[str] = None, delete: Optional[bool] = False):
    with get_session() as db:
        ...
        if not cat: await interaction.response.send_message("Category not found.", ephemeral=True); return
        if delete:
            db.delete(cat); await interaction.response.send_message("Category deleted.", ephemeral=True); return
        if new_name:
            cat.name=new_name.strip(); await interaction.response.send_message(f"Renamed to **{cat.name}**.", ephemeral=True); return
...
async def editsubcategory(interaction: discord.Interaction, category: str, old_subcategory: str, new_subcategory: Optional[str] = None, delete: Optional[bool] = False):
    with get_session() as db:
        ...
        if not cat: await interaction.response.send_message("Category not found.", ephemeral=True); return
        sub = db.query(Subcategory).filter(Subcategory.category_id==cat.id, Subcategory.name.ilike(old_subcategory.strip())).one_or_none()
        if not sub: await interaction.response.send_message("Subcategory not found.", ephemeral=True); return
        if delete: db.delete(sub); await interaction.response.send_message("Subcategory deleted.", ephemeral=True); return
        if new_subcategory: sub.name=new_subcategory.strip(); await interaction.response.send_message(f"Renamed to **{cat.name} ▸ {sub.name}**.", ephemeral=True); return
        await interaction.response.send_message("No changes provided.", ephemeral=True)
...
async def listcards(interaction: discord.Interaction, scope: str, category: Optional[str] = None, subcategory: Optional[str] = None):
    ...
    with get_session() as db:
        cat_id = sub_id = None; title = "All Categories"
        if scope == "category":
            cat = db.query(Category).filter(Category.name.ilike((category or '').strip())).one_or_none()
            if not cat: await interaction.response.send_message("Category not found.", ephemeral=True); return
            cat_id = cat.id; title = cat.name
            cards = db.query(Card).filter(Card.category_id==cat.id).order_by(Card.question.asc()).all()
        elif scope == "subcategory":
            cat = db.query(Category).filter(Category.name.ilike((category or '').strip())).one_or_none()
            if not cat: await interaction.response.send_message("Category not found.", ephemeral=True); return
            sub = db.query(Subcategory).filter(Subcategory.category_id==cat.id, Subcategory.name.ilike((subcategory or '').strip())).one_or_none()
            if not sub: await interaction.response.send_message("Subcategory not found.", ephemeral=True); return
            cat_id = cat.id; sub_id = sub.id; title = f"{cat.name} ▸ {sub.name}"
...
async def reviewcards(interaction: discord.Interaction, scope: str, category: Optional[str] = None, subcategory: Optional[str] = None):
    scope = scope.lower().strip(); user_id = interaction.user.id
    with get_session() as db:
        cat_id = sub_id = None
        if scope == "category":
            cat = db.query(Category).filter(Category.name.ilike((category or '').strip())).one_or_none()
            ...
            sub = db.query(Subcategory).filter(Subcategory.category_id==cat.id, Subcategory.name.ilike((subcategory or '').strip())).one_or_none()
            if not sub: await interaction.response.send_message("Subcategory not found.", ephemeral=True); return
            cat_id = cat.id; sub_id = sub.id
        elif scope != "all":
            await interaction.response.send_message("Scope must be one of: all, category, subcategory.", ephemeral=True); return

        exist = _candidate_cards(db, scope, cat_id, sub_id)
        if not exist: await interaction.response.send_message("No cards available for that selection.", ephemeral=True); return
        ...
        if not first: await interaction.response.send_message("No cards available.", ephemeral=True); return
        ...
        title = "Review: All Categories"
        if scope=="category" and first.category: title=f"Review: {first.category.name}"
        if scope=="subcategory" and first.subcategory: title=f"Review: {first.category.name} ▸ {first.subcategory.name}"
    ...
    active_reviews[sent.id] = {"user_id": user_id, "scope": scope, "card_id": first.id, "category_id": cat_id, "subcategory_id": sub_id}

@addcategory.autocomplete("name")
async def ac_addcategory_name(interaction: discord.Interaction, current: str): return []
...
if __name__ == "__main__":
    if not TOKEN:
        logging.error("DISCORD_TOKEN is missing (set it in Fly secrets)."); time.sleep(60); raise SystemExit(1)
    try: bot.run(TOKEN)
    except Exception: logging.exception("Fatal error during startup"); time.sleep(60); raise
